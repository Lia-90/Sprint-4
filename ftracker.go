...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	mSpeed := meanSpeed(action, duration) // средняя скорость
	runCalories := (runningCaloriesMeanSpeedMultiplier * mSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return runCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	mSpeed := meanSpeed(action, duration) * kmhInMsec // средняя скорость
	walkCalories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(mSpeed, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return walkCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код зде
	mSpeed := float64(lengthPool) * float64(countPool) / mInKm / duration // средняя скорость
	swimCalories := (mSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimCalories
}
